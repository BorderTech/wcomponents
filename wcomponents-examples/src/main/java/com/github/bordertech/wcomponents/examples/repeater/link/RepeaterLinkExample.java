...
/** 
 * These examples show different usages of buttons/links when used within
 * a {@link WRepeater} in different combinations.
 * 
 * @author Adam Millard 
 */
public class RepeaterLinkExample extends WPanel
{
    ...
    protected void preparePaintComponent(final Request request)
    {
        if (!isInitialised())
        {
            ...
            List<List<MyData>> rootList = new ArrayList<List<MyData>>();
            
            List<MyData> subList = new ArrayList<MyData>();
            subList.add(new MyData("Ernie"));
            subList.add(new MyData("Bert"));
            rootList.add(subList);
            
            subList = new ArrayList<MyData>();
