...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  -->
	<xsl:template match="xsl:template[@name]/@priority|xsl:stylesheet|xsl:import|xsl:include|xsl:strip-space|xsl:output|processing-instruction()|comment()" priority="2" />
  ...
  -->
	<xsl:template match="concat">
		<x:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
			...
			<!-- there are no keys left on core WComponents. This is for theme overrides of XSLT only. -->
