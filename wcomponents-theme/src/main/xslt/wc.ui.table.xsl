...
<!-- THEAD -->
	...
	<xsl:template match="ui:th" mode="thead">
		...
		<th id="{concat($tableId,'_thh', position())}" scope="col" data-wc-columnidx="{position() - 1}">
			...
			<xsl:if test="number($sortable) eq 1 and $sortControl">
				<xsl:call-template name="tableicon">
...
<!--
	...
	<xsl:template match="ui:tr">
		...
		<tr id="{$rowId}" data-wc-rowindex="{@rowIndex}">
			...
			<xsl:if test="@disabled"><xsl:attribute name="disabled"><xsl:text>disabled</xsl:text></xsl:attribute></xsl:if><!-- WDataTable only -->
