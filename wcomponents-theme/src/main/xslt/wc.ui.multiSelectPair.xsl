				...
				<xsl:when test="$readOnly!=1">
					...
					<span class="wc_msp_btncol">
						...
						<xsl:call-template name="multiSelectPairButton">
							...
							<xsl:with-param name="buttonText"><xsl:text>{{t '${wc.ui.multiSelectPair.i18n.button.add}'}}</xsl:text></xsl:with-param>
						</xsl:call-template>
						<xsl:call-template name="multiSelectPairButton">
							<xsl:with-param name="value" select="'aall'"/>
							<xsl:with-param name="buttonText"><xsl:text>{{t '${wc.ui.multiSelectPair.i18n.button.addAll}'}}</xsl:text></xsl:with-param>
						</xsl:call-template>
						<xsl:call-template name="multiSelectPairButton">
							<xsl:with-param name="value" select="'rem'"/>
							<xsl:with-param name="buttonText"><xsl:text>{{t '${wc.ui.multiSelectPair.i18n.button.remove}'}}</xsl:text></xsl:with-param>
						</xsl:call-template>
						<xsl:call-template name="multiSelectPairButton">
							<xsl:with-param name="value" select="'rall'"/>
							<xsl:with-param name="buttonText"><xsl:text>{{t '${wc.ui.multiSelectPair.i18n.button.removeAll}'}}</xsl:text></xsl:with-param>
						</xsl:call-template>
					</span>
					<!-- SELECTED LIST -->
