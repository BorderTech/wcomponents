 ...
 * we are not interested in keeping (such as the separator) is not stored as a submatch. To know which
...
function explodeMask (mask, strictSequence) {
	...
	while (true) {
		...
		for (let pattern in patterns) {
			...
			let patternAsRe = patterns[pattern].patternAsRe;
			if (!patternAsRe) {  // doesn't exist, lets make it
				patternAsRe = new RegExp(escapeRe(pattern));
				patterns[pattern].patternAsRe = patternAsRe;
			}

			patternAsRe.lastIndex = 0;  // safety reset so that each match commences from the beginning of the string. should be reset anyway but a rogue 'g' flag on the regexp could stuff it up
		...
		mask = mask.substring(best.index + best[0].length);
	}
	// all done
	return result;
}
export default explodeMask;
