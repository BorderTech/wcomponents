...
function Tabset() {
	...
	this.collapseAll = tabset => expandCollapseAll(tabset, false);

	this.expandAll = tabset => expandCollapseAll(tabset, true);
	...
	function getTabFor(element) {
		...
		if (panelId) {
			...
			if (tabset) {
				const tabWidget = instance.ITEM.extend("", {"aria-controls": panelId});
