...
export default initialise.register(new Image());
...
function Image() {
	...
	function makeEditButtons(element) {
		...
		if (element && element.matches(imageSelector)) {
			makeEditButton(element);
		} else {
			Array.prototype.forEach.call(el.querySelectorAll(imageSelector), makeEditButton);
		}
	}
