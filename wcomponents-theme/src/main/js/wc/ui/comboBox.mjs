...
function ComboBox() {
	...
	function focusListbox(listbox) {
		if (listbox && listbox.querySelector(optionSelector)) {
	...
	function shedSubscriber($event) {
		const element = $event.target,
			action = $event.type;
		let opener;
		if (!(element && element.matches(comboSelector))) {
			return;
		}
	...
	function shedSelectSubscriber($event) {
		let listbox, combo;
		const element = $event.target;
		if (element && element.matches(optionSelector) && (listbox = getListBox(element)) && (combo = getCombo(listbox))) {
	 ...
	 */
	this.getListWidget = () => listBoxSelector;
 ...
 *
 */
export default initialise.register(new ComboBox());
