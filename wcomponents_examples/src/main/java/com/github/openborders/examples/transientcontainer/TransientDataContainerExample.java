package com.github.openborders.examples.transientcontainer; 

import java.util.Arrays;
import java.util.List;

import com.github.openborders.AbstractTransientDataContainer;
...
/**
 * <p>An example showing best practice using the AbstractTransientDataContainer. 
 * While this example just wraps one table with the TransientDataContainer, a
 * "real" application is likely to wrap a larger section of their UI. For 
 * example, a table containing data may be wrapped to force it to be re-read
 * from an application cache each time.</p>
 * 
 * <p>This example uses a dummy cache, where data is never expired. A more
 * realistic implementation of AbstractTransientDataContainer's setupData
 * method would invoke the DMS, which will call the underlying service if
 * the result is not found in the cache.</p>
 * 
...
public class TransientDataContainerExample extends WPanel
{
    ...
    /**
     * An example implementation of the TransientDataContainer. In this example, 
     * the text to be displayed is stored as an attribute rather than a more
     * sensible value, e.g. a cache key.
     * 
     * @author Yiannis Paschalidis
     */
    private static final class ExampleTransientDataContainer extends AbstractTransientDataContainer
    {
         ...
         */
        public ExampleTransientDataContainer(final ExampleTablePanel tablePanel)
        {
        ...
        /**
         * Sets the cache key as an attribute which is persisted across requests.
         * This will be the only data that ends up being stored in the session.
         * The cache key is used to retrieve the data from the application cache
         * before each paint.
         * 
