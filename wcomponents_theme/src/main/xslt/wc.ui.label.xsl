<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ui="https://github.com/openborders/wcomponents/namespace/ui/v1.0" xmlns:html="http://www.w3.org/1999/xhtml" version="1.0">
	...
	<!--
		Creating a label is not as simple as it may appear. A HTML Label element is 
		specific in its purpose. It may only be used to label a labelable element.
		
		WLabel may be "for" something which is not a labelable element, such as a 
		multiSelectPair, or even not for anything. Therefore, in order to determine 
		what (if anything) to output we need to find out what we are labelling.
		
		ui:label is also a child of WField and as such has special output parameters
		depending upon the contents of the field's input child. This makes labels even
		more complicated.
		
		In some cases the ui:label should be output as part of another transform
		and not putput at all in-situ. The current components which pull the 
		ui:label are ui:radioButton, ui:checkBox and ui:selectToggle[@renderAs='control'].
		TODO: the variations of ui:button should also do this if the button has 
		no text content.
		...
		<xsl:choose>
			...
			<xsl:when test="$for and $for!=''">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<!-- 
							If a WLabel is "for" the ui:application it is indicative of a Java error
							in which the real labelled component is not in the WComponent render tree.
						-->
			...
			<xsl:when test="not($for)">
				<!-- 
					...
					NOTE: this is VERY slow in the Microsoft XSLProcessor (which is much faster with keys)
					but should be a very little used calculation. 
			...
			<xsl:otherwise>
				<!-- 
					This is VERY dodgy and should not be possible in WComponents. If we get here (ie @for='') then someone has
					overridden the WLabel renderer or broken the determination of the id of the component the 
					WLabel is for. In either case this is indicative of a WComponent Java error.
				-->
				<xsl:call-template name="makeLabelForNothing">
					<xsl:with-param name="style" select="$style"/>
				</xsl:call-template>
