...
/**
 * A track provides additional information relating to the video, for example
 * subtitles. Note that all themes support tracks in WebVTT format. These files
 * are encoded as UTF-8 and must be served with MIME type "text/vtt". Themes may
...
public interface Track extends ContentStreamAccess {
	...
	/**
	 * Describes the kind of track. This is used by the client to determine how
	 * the track should be handled.
	 */
	enum Kind {
		/**
		 ...
		 * sound is unavailable or not clearly audible (e.g. because it is
		 * muted, drowned-out by ambient noise, or because the user is deaf).
		 * Overlaid on the video; labeled as appropriate for the
		 * hard-of-hearing.
		 */
		CAPTIONS,
		/**
		 ...
		 * the application without a screen while driving, or because the user
		 * is blind). Synthesized as audio.
		 */
		DESCRIPTIONS,
		/**
		 * Chapter titles, intended to be used for navigating the media
		 * resource. Displayed as an interactive (potentially nested) list in
