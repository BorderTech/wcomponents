...
/**
 * A class used to make sure the HTML/XML generated by each {@link WComponent} is well formed.
 * {@link DebugUtil} is used to determine if XML Validation is enabled. As each WComponent paints
 * its output, and if XML validation is enabled, WComponent calls this class to verify the HTML/XML.
 * If a WComponent has badly formed HTML/XML, then an error message, along with the component
