...
/**
 * The interface that the Web framework needs to see from the
 * HttpServletRequest. The support for "session scope" variables should not be
 * used except for communicating between separate WComponent trees (eg in
 * separate servlets).
 * <p>
 * The {@link com.github.bordertech.wcomponents.servlet.WServlet} class is the
 * generic servlet that sits between the servlet container and the WComponent
 * framework. The WServlet class is responsible for creating Request objects
 * that represent the incoming HTTP requests, and for dispatching them to the
 * WComponent tree.
 *
...
public interface Request {
	 ...
	 *
	 * @param key a <code>String</code> containing the key/name of the parameter
	...
	/**
	 * If the request parameter is a file attachment, use this method to access
	...
	/**
	 * If the request parameter is a file attachement, use this method to access
	 * the parsed {@link FileItem}.
	 *
	 * @param key the name of the parameter used to pass the file content.
	...
	/**
	 * Indicates whether the given request contains the same set of parameters
	 * as this one.
	 *
	 * @param other the request to check.
	 * @return true if the other request contains the same parameters as this
	 * one.
	 */
	 ...
	 *
	 * <p>
	 * Attributes can be set two ways. The container may set attributes to make
	 * available custom information about a request. Attributes can also be set
	 * programatically using setAttribute.</p>
	 *
	 ...
	 * requests. If the object passed in is null, the effect is the same as
	...
	/**
	 * Returns the object bound with the specified name in the application
	 * session, or <code>null</code> if no object is bound under the name.
	 * <p>
	 * This method is relevant for portlets in which case getAppSessionAttribute
	 * accesses a portlet scoped session, while getSessionAttribute accesses the
	 * global session.
	 *
	 * @param key the session attribute key
	 * @return an <code>Object</code> containing the value of the attribute, or
	 ...
	 * <p>
	 * If the value passed in is null, this has the same effect as removing the
	 * attribute.
	 *
	 * @param key the session attribute key.
	 * @param value an <code>Object</code> containing the value of the
	 * attribute.
	 */
	void setSessionAttribute(String key, Serializable value);

	/**
	 * Binds an object to the application session, using the name specified. If
	...
	/**
	 * Gets the parameter for the given key. If no such parameter is defined,
	 * returns null. This method differs from Parameters.getInstance().get(key)
	 * in that the value returned may be customised to suit the "app". In a
	 * portlet environment an "app" is represented by a portlet.
	 *
	 ...
	 * The container should react by invalidating the session and redirecting
	 * the client to some suitable page.
	 *
	 * @see
	 * com.github.bordertech.wcomponents.container.AbstractContainerHelper#redirectForLogout()
	 */
	void logout();
	...
	/**
	 * Triggers the publishing event using the <code>value</code> as the event
	 * payload.
	 *
	...
	/**
	 * Triggers the publishing event using the <code>eventMap</code> as the
	 * event payload.
	 *
	 * @param action name of the publishing event to trigger
	 ...
	 *
	 * @return an int specifying the maximum inactive interval in seconds.
	 * Negative if the session never expires.
	 */
