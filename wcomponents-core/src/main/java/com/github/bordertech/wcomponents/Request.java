...
/**
 * The interface that the Web framework needs to see from the HttpServletRequest. The support for
 * "session scope" variables should not be used except for communicating between separate WComponent
 * trees (eg in separate servlets).
 * <p>
 * The {@link com.github.bordertech.wcomponents.servlet.WServlet} class is the generic servlet that
 * sits between the servlet container and the WComponent framework. The WServlet class is
 * responsible for creating Request objects that represent the incoming HTTP requests, and for
 * dispatching them to the WComponent tree.
 *
...
public interface Request {
	 ...
	 *
	 * @param key a <code>String</code> containing the key/name of the parameter whose value is
	 * requested
	...
	/**
	 * If the request parameter is a file attachment, use this method to access the content of the
	 * attached file.
	 *
	...
	/**
	 * If the request parameter is a file attachement, use this method to access the parsed
	 * {@link FileItem}.
	 *
	 * @param key the name of the parameter used to pass the file content.
	...
	/**
	 * @return an <code>Enumeration</code> of <code>String</code> objects containing the names of
	 ...
	 * no attribute of the given name exists.</p>
	 *
	 * <p>
	 * Attributes can be set two ways. The container may set attributes to make available custom
	 * information about a request. Attributes can also be set programatically using
	 * setAttribute.</p>
	 *
	...
	/**
	 * Stores an attribute in this request. Attributes are reset between requests. If the object
	 * passed in is null, the effect is the same as removing the attribute.
	 *
	...
	/**
	 * Returns the object bound with the specified name in the application session, or
	 * <code>null</code> if no object is bound under the name.
	 * <p>
	 * This method is relevant for portlets in which case getAppSessionAttribute accesses a portlet
	 * scoped session, while getSessionAttribute accesses the global session.
	 *
	 * @param key the session attribute key
	 * @return an <code>Object</code> containing the value of the attribute, or <code>null</code> if
	 * the attribute does not exist
	...
	/**
	 * Binds an object to the application session, using the name specified. If an object of the
	...
	/**
	 * Gets the parameter for the given key. If no such parameter is defined, returns null. This
	 * method differs from Parameters.getInstance().get(key) in that the value returned may be
	 * customised to suit the "app". In a portlet environment an "app" is represented by a portlet.
	 *
	...
	/**
	 * Signal to the servlet that we want to log out (terminate the session). The container should
	 * react by invalidating the session and redirecting the client to some suitable page.
	 *
	 ...
	 *
	 * @return an int specifying the maximum inactive interval in seconds. Negative if the session
	 * never expires.
	 */
