...
/**
 * <p>
 * The WImage component provides a way for applications to display images within
 * their application. It may either serve up a pre-defined image which is part
 * of the application, or generate the image dynamically.
 * </p>
 *
 * <pre>
...
public class WImage extends WBeanComponent implements Targetable, AjaxTarget {
	...
	/**
	 * <p>
	 * Creates a WImage with the given static content. This is provided as a
	 * convenience method for when the image is included as static content in
	 * the class path rather than in the web application's resources.
	 * </p>
	 * <p>
	 * The mime type for the image is looked up from the "mimeType.*" mapping
	 * configuration parameters using the resource's file extension.
	 * </p>
	 *
	...
	/**
	 * Creates a dynamic URL that the image can be loaded from. In fact the URL
	 ...
	 * handleRequest method below detects this when the browser requests the
	 * image
	 *
	...
	/**
	 * When an img element is included in the html output of a page, the browser
	 * will make a second request to get the image contents. The handleRequest
	 * method has been overridden to detect whether the request is the "image
	 * content fetch" request by looking for the parameter that we encode in the
	 * image url.
	 *
	...
	/**
	 * Retrieves the cache key for this image. This is used to enable caching of
	 * the image on the client agent.
	 *
	 * @return the cacheKey
	 */
	 ...
	 * <p>
	 * Returns the size set via {@link #setSize(Dimension)}. If this has not
	 * been set and an image resource is provideing the image then the size of
	 * the image resource is returned. Otherwise return null.
	 * </p>
	 *
	 ...
	 * <p>
	 * Returns the alternative text set via {@link #setAlternativeText(String)}.
	 * If this has not been set and an image resource is providing the image
	 * then the description of the image resource is returned. Otherwise return
	 * null.
	 * </p>
	 *
