...
/**
 * <p>
 * The ImageResourceMap map keeps a reference to all internal resources within
 * an application. It is used by the WContentHelperServlet to efficiently serve
 * binary data without having to go through normal WComponent processing.</p>
 *
 * Two things stop this map from consuming excessive amounts of memory:
 * <ol>
 * <li>InternalResources are files which are present in the classpath. There
 * will be a finite set.</li>
 * <li>The InternalResource implementation does not hold the file data, just a
 * reference to the file.</li>
 * </ol>
 *
...
public final class InternalResourceMap {
	...
	/**
	 * A map of internal resources cache keys by path. This is used to bust the
	 * browser cache when a resource changes after a new deployment of the
	 * application.
	 */
