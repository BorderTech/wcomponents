 ...
 * </p>
 * <p>
 * If one or more file types is set {@link #setFileTypes(java.util.List)}, then each uploaded file will be validated against the accepted list. If
 * accepted list contains any extension(s) then uploaded files will be first checked against them. Developers can choose to have custom validation of
 * file by retrieving it's MIME type, see {@link #getMimeType()}.
 * </p>
 *
...
public class WFileWidget extends AbstractInput implements AjaxTarget, SubordinateTarget {
	...
	/**
	 * Determines the file types accepted by this widget. Note that duplicates are not allowed and these are not case sensitive.<br>
	 * The file type(s) can be either:
	 * <ul>
	 * <li><strong>MIME type</strong></li>
	 * <li><strong>Extension</strong></li>
	 * </ul>
	 * <strong>MIME type</strong>: it is <em>type/subtype</em>, where <em>type</em> is <code>text, image, application</code> etc, and <em>subtype</em>
	 ...
	 * <br>
	 * <strong>Extension</strong>: A string whose first character is a "." (U+002E) character (Indicates that files with the specified file extension
	 * are accepted). Some example extensions are:
	 * <ul>
	 ...
	 *
	 * @param types The file types that will be accepted by the file input. Note that this is not additive, it will overwrite any previously set
	...
	/**
	 * Indicates whether the uploaded file is valid. If {@link #getMaxFileSize()} is set then it is validated, otherwise {@link #getFile()} is
	 * considered valid.
	 *
