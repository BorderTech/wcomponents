...
/**
 * Adapter to allow classes that implement the {@link BasicTableModel} interface
 * to be used as the model for {@link WTable}.
 * <p>
 * {@link BasicTableModel} provides a basic interface for data that does not
 * have a tree like structure (ie not expandable). Therefore, for data that is
 * not expandable, it is recommended to use this adapter and
...
public class AdapterBasicTableModel implements TableModel, Serializable {
	 ...
	 * {@link AdapterBasicTableModel} for {@link WTable}. This model is used for
	 * data that is not in a tree like structure (ie not expandable).
	 * </p>
	 * <p>
	 * As the data is not expandable, the interface only requires a single row
	 * index, instead of being like the {@link TableModel} interface that uses a
	 * list of indexes.
	 * </p>
	 * <p>
