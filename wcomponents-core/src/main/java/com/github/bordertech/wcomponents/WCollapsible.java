...
/**
 * <p>
 * The WCollapsible component enables a given component to be expanded/collapsed
 * (shown/hidden) via clicking on the collapsible's header section. When a user
 * clicks the heading, the visibility of the collapsible's content is
 * toggled.</p>
 *
 * <p>
 * Various {@link CollapsibleMode modes of operation} are supported, which allow
 * developers to tune performance by only loading content when it is needed.</p>
 *
...
public class WCollapsible extends AbstractNamingContextContainer implements AjaxTarget, SubordinateTarget, Marginable {
	...
	public static enum CollapsibleMode {
		/**
		 * Indicates that expanding/collapsing involves a round-trip to the
		 * server. The collapsible content will be rendered only when the
		 * collapsible is open.
		 *
		 * @deprecated Use CollapsibleMode DYNAMIC instead as a like-for-like
		 * replacement or any other mode if it is more appropriate to the
		 * individual use case.
		 */
		SERVER,
		/**
		 * Indicates that expanding/collapsing is performed client-side, using a
		 * single AJAX request to obtain the content the first time the
		 * collapsible is expanded.
		 */
		LAZY,
		/**
		 * Indicates that expanding/collapsing is handled on the client. The
		 * content is always rendered, and no additional trips to the server are
		 * made.
		 */
		CLIENT,
		/**
		 * Indicates that expanding/collapsing is performed using AJAX requests.
		 * A request will be made every time the collapsible is expanded.
		 */
		DYNAMIC,
		/**
		 * Indicates that expanding/collapsing is performed client-side, using a
		 * single AJAX request to obtain the content immediately after the page
		 * loads.
		 */
		EAGER
	};
	 ...
	 * @param heading the collapsible's heading.
	 * @param mode the mode of the collapsible
	 * @param group the {@link CollapsibleGroup} that this collapsible belongs
	 * to.
	 */
	public WCollapsible(final WComponent content, final String heading, final CollapsibleMode mode,
	 ...
	 *
	 * @return true if the collapsible is collapsed for the given user, false if
	 * expanded.
	 */
	 ...
	 *
	 * @param collapsed true if the collapsible is to be collapsed for the given
	 * user, false if expanded.
	 */
	...
	/**
	 * @return the decorated label that is used to render the collapsible
	 * heading.
	 */
	...
	/**
	 * The group name that this collapsible component belongs to. If it is not
	 * part of a group then the name defaults to {@link WComponent#getName()}.
	 *
	...
	/**
	 * Set the {@link CollapsibleGroup} that this component belongs to. This
	 * will enable a {@link WCollapsibleToggle} component to target the group.
	 *
	...
	/**
	 * Override handleRequest to perform processing necessary for this
	 * component. This is used to handle the server-side collapsible mode and to
	 * synchronise with the client-side state for the other modes.
	 *
	...
	public static class CollapsibleModel extends ComponentModel {
		...
		/**
		 * The CollapsibleGroup, primarily used with a
		 * {@link #CollapsibleToggle} to toggle a group of collapsibles at once.
		 */
