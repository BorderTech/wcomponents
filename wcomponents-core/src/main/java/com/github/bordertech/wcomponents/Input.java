...
public interface Input extends WComponent, Disableable, Mandatable, DataBound, BeanBound, BeanProviderBound {
	...
	// Validation
	/**
	 * Adds a validator to the input field. Validators are not automatically called on form
	 * submission. Using a {@link ValidatingAction} or an explicit call to
	 * {@link WComponent#validate(List)} will trigger validation on a WComponent tree.
	 *
	...
	// Action on change
	/**
	 * Sets the action that you want run if the input is changed by the user. The action command
	 * will be set to the new value of the input.
	 *
	...
	/**
	 * Returns the data object that has been associated with this input component, else null. For
	...
	/**
	 * Associates this input with a data object that can be easily accessed in the execute() method
	...
	/**
	 * Same as {#getValueAsString()}. This method exists simply to clarify the relationship between
	 * the input component, its Action, and the ActionEvent sent to the execute() method of the
	 * Action.
	 *
	...
	// Mandatory flag
	/**
	 * Set whether or not this input is mandatory, and customise the error message that will be
	 * displayed.
	 *
	 ...
	 * @return the value of this component on the Request, or its current state if it is not on the
	 * request.
	 */
	 ...
	 * <p>
	 * If required, this method can convert the data into the correct type and also do any
	 * validation before the value is used.
	 * </p>
	 *
	...
	/**
	 * Indicates whether this input field is empty. An input is considered empty if the value is
