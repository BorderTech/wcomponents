...
public final class WebUtilities {
	...
	public static String percentEncodeUrl(final String urlStr) {
		...
		try {
			...
			String tmp = URLDecoder.decode(urlStr, StandardCharsets.UTF_8.name());
			// Encode as per legacy HttpClient URI class for backward compatability
			String encoded = percentEncodeUrlLegacy(tmp);
			// Check URL is valid
			java.net.URI uri = new java.net.URI(encoded);
			return uri.toString();
		} catch (Exception e) {
			throw new IllegalStateException("Could not percent encode URL [" + urlStr + "]", e);
		}
	}

	/**
	 * Percent encode a URL as per the HTTPClient URI class that handled unescaped URLs.
	 *
	 * @param urlStr the unescaped URL to encode
	 * @return the URL percent encoded
	 */
	private static String percentEncodeUrlLegacy(final String urlStr) {

		if (Util.empty(urlStr)) {
			return urlStr;
		}

		String tmp = urlStr.trim();
		...
		// Handle path
		if (at < length) {
			// Check for first delim (query or fragment)
			...
			String path = tmp.substring(0, next);
			url.append(path);
			at = next;
		}
		...
		if (at >= 0 && at + 1 < length && tmp.charAt(at) == '?') {
			...
			String query = escapeForUrlLegacy(tmp.substring(at + 1, next));
		...
		if (at >= 0 && at + 1 <= length && tmp.charAt(at) == '#') {
			...
			if (at + 1 == length) {
				...
				fragment = escapeForUrlLegacy(tmp.substring(at + 1));
			}
	...
	/**
	 * Escapes the given string as per the legacy HttpClient URI class.
	 *
	 * @param input the String to escape.
	 * @return an escaped copy of the string.
	 */
	private static String escapeForUrlLegacy(final String input) {
		...
		for (int i = 0, len = input.length(); i < len; ++i) {
			...
			// Unreserved chars from HTTPClient V3.1 URI
			if ((ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z') || (ch >= '0' && ch <= '9')
					...
					|| ch == ')') {
				buffer.append(ch);
				// Reserved chars from HTTPClient V3.1 URI
			} else if (ch == ';'
					|| ch == '/'
					|| ch == '?'
					|| ch == ':'
				...
				buffer.append('%').append(Integer.toHexString(0xc0 | (ch >> 6)).toUpperCase());
				buffer.append('%').append(Integer.toHexString(0x80 | (ch & 0x3F)).toUpperCase());
			} else {
				buffer.append('%').append(Integer.toHexString(0xe0 | (ch >> 12)).toUpperCase());
				buffer.append('%').append(Integer.toHexString(0x80 | ((ch >> 6) & 0x3F)).toUpperCase());
				buffer.append('%').append(Integer.toHexString(0x80 | (ch & 0x3F)).toUpperCase());
			}
		}
