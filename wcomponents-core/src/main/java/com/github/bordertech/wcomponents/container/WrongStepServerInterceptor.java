...
/**
 * This wrong step interceptor prevents an old request being processed that
 * might have been submitted by a user using multiple windows.
 * <p>
 * If a step error occurs, then the user, depending on the redirect flag, is
 * either (1) redirected to an error page or (2) warped to the future by the
 * application being rendered in its current state. When the user is warped to
 * the future, the handleStepError method is called on WApplication, which
 * allows applications to take the appropriate action for when a step error has
 * occurred.
 * </p>
 *
