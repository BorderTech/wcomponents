package com.github.dibp.wcomponents.test.example; 
...
/** 
 * <p>This class is an example showing various ways of testing a 
 * WComponent UI using Selenium. The same UI is tested multiple
 * times using different approaches.</p>
 * 
...
{
    /**
     * Creates a ExampleSeleniumTest. The WComponent which is passed to
     * the superclass's constructor is the UI to be tested. Small 
     * sections of application code may need to be wrapped in a shell
     ...
     * describe a path through the WComponent Tree structure. This is the preferred
     * method of testing WComponent UIs, as it should be stable between releases,
     * and does not require applications to expose UI structure via getters.
     * 
    ...
    /**
     * This test implementation uses a 'traditional' Selenium approach,
     * matching on HTML elements. This method should not generally be
     * used for testing WComponent applications, as the HTML output
     * can change without notice between different theme versions;
     * the rendered HTML is transform-dependant, not WComponent-dependant.
     */      
    @Test
     ...
     * tested has been built with getters for every control, which is 
    ...
    /**
     * <p>A simple UI to test, which contains a WTabSet with two tabs.
     * The first tab is client-side, so is always present. The second
     * tab is lazy-loaded via AJAX.</p>
     * 
     * <p>The first tab contains an example where the fields are not exposed via getters.
     * The second tab contains the same example, but with getters.</p>
     */
    ...
    /**
     * A re-implementation of the {@link com.github.dibp.wcomponents.examples.TextDuplicator} example.
     */
    private static class TextDuplicator extends WContainer
    {
        /** The text field which the actions modify the state of. */
        protected final WTextField textFld = new WTextField();

        /** The text field which the actions modify the state of. */
        protected final WButton duplicateButton = new WButton("Duplicate");

        /** The text field which the actions modify the state of. */
        protected final WButton clearButton = new WButton("Clear");
