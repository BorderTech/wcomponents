...
/**
 * <p>An implementation of By which can find HTML elements which correspond to (most) 
 * WComponents. Only WComponents which emit elements with ids can be searched on.
 * This means that components such as WText and "WComponent" itself can not be
 * used in a search path.</p>
 * 
 * <p><b>Note:</b> Since there's no mapping from XHTML to the WComponent XML schema, 
 * this {@link By} implementation will always search from the root component, no matter what 
 * the search context is.</p> 
 *
 * <p>See {@link TreeUtil#findWComponents(WComponent, String[])} for details
 * on the path syntax.</p>
 * 
...
public class ByWComponentPath extends By
{
    ...
    public List<WebElement> findElements(final SearchContext searchContext)
    {
        ...
        {
            WComponent component = comp.getComponent();
            UIContext context = comp.getContext();
            UIContextHolder.pushContext(context);
            ...
            {
                SeleniumUtil.narrowResults(resultForComp, component, context, value);
