...
public final class LdeSessionUtil
{
    ...
    private static final String LDE_PERSIST_SESSION_PARAM = "wcomponent.lde.session.persist";
    
    /** The key used to look up the LDE persistant session load flag in the {@link Config WComponent Configuration}. */ 
    private static final String LDE_LOAD_PERSISTANT_SESSION_PARAM = "wcomponent.lde.session.loadPersisted";
    ...
    public static synchronized void serializeSessionAttributes(final HttpSession session)
    {
        if (session != null)
        {
            ...
            if (!file.exists() || file.canWrite())
            {
                ...
                for (Enumeration keyEnum = session.getAttributeNames(); keyEnum.hasMoreElements();)
                {
                    ...
                    {
                        log.error("Skipping attribute \"" + attributeName + "\" as it is not Serializable");
                    }
                }
