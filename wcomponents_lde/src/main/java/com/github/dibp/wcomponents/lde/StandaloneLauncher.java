 ...
 * i.e. directly From a jar file rather than through an IDE. It 
 * provides basic controls to interact with the LDE. 
 * 
...
public class StandaloneLauncher
{
    ...
    private StandaloneLauncher()
    {
        ...
        content.add(new JScrollPane(log, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED), BorderLayout.CENTER);
        ...
        button.addActionListener(new ActionListener()
        {
            public void actionPerformed(final ActionEvent event)
            {
                ...
                try
                {
                    ...
                    clazz.getMethod("browse", URI.class).invoke(instance, new URI(getInstance().launcher.getUrl()));
                }
                catch (ClassNotFoundException e)
                {
                    //Desktop isn't available in Java 1.5, so we'll just use a windows specific cmdline for now
                    String cmd = "rundll32 url.dll,FileProtocolHandler " + getInstance().launcher.getUrl();
    ...
    public static void main(final String[] args) throws Exception
    {
        ...
        System.setProperty("org.apache.commons.logging.Log", "com.github.dibp.wcomponents.lde.StandaloneLauncher$TextAreaLogger");
        ...
        config.setProperty("wcomponent.lde.server.port", 0);
