...
import com.github.dibp.wcomponents.util.mock.servlet.MockHttpSession;
...
public class DevToolkit_Test
{
    ...
    public void setUp()
    {
        ...
        Config.getInstance().setProperty("wcomponent.lde.devToolkit.enabled", "true");
    }
    ...
    {
        Config.getInstance().clearProperty("wcomponent.lde.devToolkit.enabled");
        Assert.assertFalse("DevToolkit should be disabled by default", DevToolkit.isEnabled());

        Config.getInstance().setProperty("wcomponent.lde.devToolkit.enabled", "false");
    ...
    public void testPaintHeaderShowConfig()
    {
        ...
        String header = renderHeader(toolkit);
        Assert.assertTrue("Should not contain config", header.indexOf("WComponent configuration") == -1);

        sendToolkitRequest(toolkit, "devToolkit_showConfig", "true");
        header = renderHeader(toolkit);
        Assert.assertTrue("Should contain config", header.indexOf("WComponent configuration") != -1);
        ...
        for (Iterator i = Config.getInstance().getKeys() ; i.hasNext() ; )
        {
            ...
            Assert.assertTrue("Should contain key " + key, header.indexOf(WebUtilities.encode(key)) != -1);
            Assert.assertTrue("Should contain value " + value, header.indexOf(WebUtilities.encode(value)) != -1);
        }
    }
