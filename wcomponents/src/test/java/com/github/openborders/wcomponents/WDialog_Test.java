...
import com.github.openborders.wcomponents.WDialog.DialogModel;
...
public class WDialog_Test extends AbstractWComponentTestCase
{
    ...
    public void testSetTitle()
    {
        ...
        resetContext();
        Assert.assertEquals("shared title should be as initially set", TEST_TITLE, dialog.getTitle());
    }
    ...
    public void testIsAjaxTargeted()
    {
        ...
        Assert.assertFalse("Should not be targeted if there is no AJAX operation", dialog.isAjaxTargeted());
        
        setActiveAjaxOperation(appPanel);
        Assert.assertFalse("Should not be targeted if targeting another component", dialog.isAjaxTargeted());        
        ...
        Assert.assertTrue("Should be targeted when a child component is targeted", dialog.isAjaxTargeted());        
    }
    ...
    private void setActiveAjaxOperation(final WComponent target)
    {
        ...
        AjaxHelper.setCurrentOperationDetails(operation, new ComponentWithContext(target, UIContextHolder.getCurrent()));
    }
}
