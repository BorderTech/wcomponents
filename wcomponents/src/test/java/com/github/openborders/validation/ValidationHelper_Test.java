...
import com.github.openborders.validation.ValidationHelper;
...
public class ValidationHelper_Test
{
    ...
    public void testExtractDiagnostics()
    {
        ...
        // Test when empty
        Assert.assertEquals("Should not have any error diagnostics", 0, ValidationHelper.extractDiagnostics(diags, Diagnostic.ERROR).size());
        Assert.assertEquals("Should not have any warning diagnostics", 0, ValidationHelper.extractDiagnostics(diags, Diagnostic.WARNING).size());
        Assert.assertEquals("Should not have any info diagnostics", 0, ValidationHelper.extractDiagnostics(diags, Diagnostic.INFO).size());
        ...
        List<Diagnostic> extractedDiags = ValidationHelper.extractDiagnostics(diags, Diagnostic.ERROR);
        ...
        Assert.assertEquals("Should have one error diagnostic", 1, extractedDiags.size());
        Assert.assertTrue("Extracted diags should contain warning diag", extractedDiags.contains(warn));
