...
public class WTable extends WBeanComponent implements Container, AjaxTarget, SubordinateTarget, Marginable, NamingContextable
{
    ...
    public void setRowsPerPageOptions(final List<Integer> rowsPerPageOptions)
    {
        ...
        else
        {
            ...
            model.rowsPerPageOptions = new ArrayList<Integer>(rowsPerPageOptions);
    ...
    protected void addPrevExpandedRow(final Object rowKey)
    {
        ...
        {
            model.prevExpandedRows = new HashSet<Object>();
        }
    ...
    protected void addPrevRenderedRow(final Object rowKey)
    {
        ...
        {
            model.prevRenderedRows = new HashSet<Object>();
        }
    ...
    private void handleSelectionRequest(final Request request)
    {
        ...
        {
            newSelections = new HashSet<Object>(selectedRows.length);
        }
        else
        {
            if (singleSelect && selectedRows.length > 0)
            {
                ...
                newSelections = new HashSet<Object>(1);
            }
            else
            {
                // For multi-select, we need the entries for the current page only
                newSelections = new HashSet<Object>(oldSelections);
    ...
    private void handleExpansionRequest(final Request request)
    {
        ...
        Set<Object> newExpansions = null;

        TableModel model = getTableModel();

        if (model.getRowCount() == 0)
        {
            newExpansions = new HashSet<Object>();
            expandedRows = new String[0];
        }
        else if (getPaginationMode() == PaginationMode.NONE || getPaginationMode() == PaginationMode.CLIENT
                 || oldExpansions == null || !isPaginated())
        {
            newExpansions = new HashSet<Object>(expandedRows.length);
        }
        else
        {
            // row expansions only apply to the current page
            newExpansions = new HashSet<Object>(oldExpansions);
     ...
     * 
     * @param original The string array from which the null/empty strings should be removed from.
    ...
    private List<RowIdWrapper> getRowIds(final int startIndex, final int endIndex, final boolean forUpdate)
    {
        ...
        List<RowIdWrapper> rowIds = new ArrayList<RowIdWrapper>(endIndex - startIndex + 1);
        ...
        {
            // Create top level
            List<Integer> rowIndex = new ArrayList<Integer>(1);
                                    ...
                                    final boolean forUpdate, final boolean editable)
    {
        ...
        {
            // Add next level
            List<Integer> nextRow = new ArrayList<Integer>(row.getRowIndex());
            nextRow.add(i);
            // Create Wrapper
    ...
    private Map<List<Integer>, Object> getCurrentRowIndexAndKeys()
    {
        ...
        Map<List<Integer>, Object> rows = new HashMap<List<Integer>, Object>(wrappers.size());
        for (RowIdWrapper wrapper : wrappers)
        {
     ...
     * @since 1.0.0
     */
    public static interface ScrollableTableModel extends TableModel
    {
        /**
     ...
     * @since 1.0.0
     */
    public static interface BeanBoundTableModel extends TableModel, BeanProviderBound
    {
    ...
    public static class RowIdWrapper
    {
        ...
        private final List<RowIdWrapper> children = new ArrayList<RowIdWrapper>();
