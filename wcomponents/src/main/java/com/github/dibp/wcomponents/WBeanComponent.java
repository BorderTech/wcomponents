 ...
 * <p>
 * A fix has been made to the logic for a bean container, that has a bean property set, to pass the correct bean
 * value to its child bean components. This fix is an "opt in". To enable the correct bean container logic set the
 * parameter "wcomponent.bean.logic.correct=true".
 * </p>
 * 
...
public class WBeanComponent extends AbstractWComponent implements DataBound, BeanBound, BeanProviderBound
{
     ...
     */
    @Deprecated
    private static boolean CORRECT_PARENT_BEAN_LOGIC = Config.getInstance().getBoolean("wcomponent.bean.logic.correct",
                                                                                       true);
        ...
        if (bean == null)
        {
            ...
            if (beanProvider == null)
            {
                ...
                if (parent != null)
                {
                    ...
                    {
                        if (parentBeanProperty != null && !".".equals(parentBeanProperty) && parentBeanProperty.contains("."))
                        {
                            log.warn("Possible bean property logic error with bean property [" + parentBeanProperty + "]. Check runtime parameter " + CORRECT_PARENT_BEAN_LOGIC + ".");
                        }
                        bean = parent.getBean();
                    }
                }
     ...
     * This method of bean association is discouraged, as the bean will be 
     * stored in the user's session. A better alternative is to provide
     * a BeanProvider and a Bean Id.
     * 
     ...
     * This method will be used by a {@link BeanProvider}
     ...
     * The bean property is expressed in Jakarta PropertyUtils bean notation,
     * with an extension of "." to indicate that the bean itself should be used.
     * 
    ...
    /**
     * Sets the {@link BeanProvider} associated with this WBeanComponent. 
     * The bean provider will be called to supply the bean whenever necessary.
     * 
    ...
    public void setBeanProvider(final BeanProvider beanProvider)
    {
        ...
        {
            throw new SystemException("Unable to store bean provider in user's session as it is not serializable: " + beanProvider.getClass());
        }        
    ...
    /**
     * Retrieves the bean value. The value is (temporarily) cached in 
     * the scratch map to speed up subsequent accesses.
     * 
     ...
     * For bean aware components, this should only be called from 
