...
import com.github.dibp.wcomponents.util.I18nUtilities;
import com.github.dibp.wcomponents.util.ReflectionUtil;
import com.github.dibp.wcomponents.util.Util;

/**
 * <p>Holds the extrinsic state information of a WComponent. 
 * Subclasses can extend this class and add extra model attributes.</p>
 * 
 * <p>Subclasses must adhere to the following rules:
 * <ul>
...
public class ComponentModel implements WebModel, Externalizable
{
    ...
    private static final Map<Class<?>, Field[]> FIELDS_BY_CLASS = new HashMap<Class<?>, Field[]>();
    ...
    /** 
     * <p>The bit-mask for the flag that indicates whether that user data has been set.
     * This is necessary, as some components do not contain enough values to determine this automatically.
     ...
     * 
     * <p><b>Note:</b> This bit mask must never be set on a component's shared model.</p>
     */
    ...
    /** 
     * When initially constructed or deserialized, some of our fields
     * may be defaulted to the shared model, but we don't yet have a
     * reference to it. When a reference is supplied using the
     * setSharedModel method, this list controls which fields should
     * have the values set to the shared model's values.
     */
    private transient List<Field> unsetFields = Arrays.asList(getFields(this));

    /** 
     * A reference to the sharedModel. This is not serialized,
     * and must therefore be supplied after deserialization. 
     */
    ...
    /**
     * You can override the children on a per session basis. When children is set to null we know there is no
     * override. An empty array indicates no children.
     */
    ...
    /**
     * Adds a toolTip to the component. Browsers will display this value when the element is hovered-over or is in focus.
     */
     ...
     * that subclasses do not need to extend the <code>ComponentModel</code> and override the extrinsic state
     * management (eg, newComponentModel(), etc).
     */
     ...
     * 
     * Subclasses should not need to override this method, as reflection 
     * is used to read in all fields.
     * 
    ...
    /**
     * Sets the shared component model. If there are any unset fields,
     * they are set from the values in the shared model.
     *  
    ...
    /**
     * Creates a copy of mutable data, to ensure updates to one
     * component model do not update the other.
     * 
    ...
    protected Object copyData(final Object data)
    {
        ...
        {
            String[] copy = new String[((String[]) data).length];
            System.arraycopy(data, 0, copy, 0, copy.length);
            return copy;
        }
        else if (data instanceof int[])
        {
            int[] copy = new int[((int[]) data).length];
            System.arraycopy(data, 0, copy, 0, copy.length);
            return copy;
        }
        else if (data instanceof long[])
        {
            long[] copy = new long[((long[]) data).length];
            System.arraycopy(data, 0, copy, 0, copy.length);
            return copy;
        }
        else if (data instanceof float[])
        {
            float[] copy = new float[((float[]) data).length];
            System.arraycopy(data, 0, copy, 0, copy.length);
            return copy;
        }
        else if (data instanceof double[])
        {
            double[] copy = new double[((double[]) data).length];
            System.arraycopy(data, 0, copy, 0, copy.length);
            return copy;
        }
        else if (data instanceof boolean[])
        {
            boolean[] copy = new boolean[((boolean[]) data).length];
            System.arraycopy(data, 0, copy, 0, copy.length);
            return copy;
        }
        else if (data instanceof char[])
        {
            char[] copy = new char[((char[]) data).length];
            System.arraycopy(data, 0, copy, 0, copy.length);
            return copy;
        }
    ...
    /**
     * <p>Implementation of the Externalizable interface to ensure that we don't
     * serialize redundant data (anything contained by the {@link #sharedModel}.</p>
     * 
     * <p>Data is obtained using reflection so that subclasses don't need concern
     * themselves with the ComponentModel serialization mechanism.</p> 
     * 
     * <p>After the ComponentModel data is read in, some fields may be in a "unset"
     * state. The static model needs to be supplied using {@link #setSharedModel(ComponentModel)}
     * 
    ...
    public void readExternal(final ObjectInput in) throws IOException, ClassNotFoundException
    {
        ...
        unsetFields = new ArrayList<Field>();
        ...
        for (int i = 0; i < fields.length; i++)
        {
            ...
            {
                log.error("Failed to read field " + fields[i].getName(), e);
            }
        }
    }
    ...
    public void writeExternal(final ObjectOutput out) throws IOException
    {
        ...
        for (int i = 0; i < fields.length; i++)
        {
            try
            {
                ...
                else if (unsetFields != null && unsetFields.contains(fields[i]))
                {
                ...
                {
                    Object sharedValue = fields[i].get(sharedModel);
                    Object value = fields[i].get(this);
            ...
            {
                log.error("Failed to write field " + fields[i].getName(), e);
            }
        }
    }
     ...
     */
    private static final Field[] getFields(final ComponentModel model)
    {
    ...
    {
        /**
         * Compares two objects for order. Returns a negative integer,
