...
import com.github.dibp.wcomponents.util.RowIdList;
import com.github.dibp.wcomponents.util.TreeNode;
import com.github.dibp.wcomponents.util.Util;

/**
 * <p>The WDataTable component is used to display tabular data. 
 * It supports common functions such as sorting and pagination of data.</p>
 * 
 * <p>The WDataTable component is only concerned with how the UI functions, not the
 ...
 * 
 * <p>Columns may only be added statically to the table, but individual columns
 * can be shown/hidden per user by toggling their visibility. See {@link #getColumn(int)}
 * and {@link WComponent#setVisible(boolean)}.</p> 
 * 
...
public class WDataTable extends WBeanComponent implements Disableable, Container, AjaxTarget, SubordinateTarget, NamingContextable
{
     ...
     * Bounds checking is not performed, see {@link #getColumnCount()}.
     * 
    ...
    /**
     * Updates the bean using the table data model's {@link TableDataModel#setValueAt(Object, int, int)} method.
     * This method only updates the data for the current page.
     */
    ...
    /**
     * <p>Sets whether row headers should be displayed.</p>
     * 
     * <p>To set the column heading for the row headers, use:
    ...
    /**
     * Sets the table sort mode. The data model controls 
    ...
    public void setSelectedRows(final List<Integer> selectedRows)
    {
        ...
        {
            model.selectedRows = new ArrayList<Integer>(selectedRows);
    ...
    /**
     * Indicates whether the sort on this table is ascending. Note that
     * a return value of false does not necessarily indicate a descending
     * sort - see {@link #isSorted()}. 
     * 
    ...
    public List<WButton> getActions()
    {
        ...
        List<WButton> buttons = new ArrayList<WButton>(numActions);
    ...
    private void handleSelectionRequest(final Request request)
    {
        ...
        if (getDataModel().getRowCount() == 0)
        {
            ...
            selectedRows = new String[0];
                  ...
                  || oldSelections == null)
        {
            newSelections = new ArrayList<Integer>(selectedRows.length);
        }
        else
        {
            if (singleSelect && selectedRows.length > 0)
            {
                // For single-select, we need to remove the old entries
                newSelections = new ArrayList<Integer>(1);
            }
            else
            {
                // For multi-select, we need to entries for the current page only
                newSelections = new ArrayList<Integer>(oldSelections);
    ...
    /**
     * Retrieves the starting row index for the current page. 
     * Will always return zero for tables which are not paginated.
     *  
    ...
    /**
     * Retrieves the ending row index for the current page. 
     * Will always return the row count minus 1 for tables which are not paginated.
     *  
     * @return the starting row index for the current page.
     */
    ...
    private void handleExpansionRequest(final Request request)
    {
        ...
        if (model.getRowCount() == 0)
        {
            ...
            return;
                  ...
                  || oldExpansions == null)
        {
            expansions = new ArrayList<Integer>(expandedRows.length);
        }
        else
        {
            // row expansions only apply to the current page
            expansions = new ArrayList<Integer>(oldExpansions);
     ...
     * 
     * @param original
     *            The string array from which the null/empty strings should be removed from.
     * @return Array of non empty strings from the <code>original</code> string array.
     */
    private String[] removeEmptyStrings(final String[] original)
    {
        if (original == null)
        {
            return null;
    ...
    /**
     * A naming context is only considered active if it has been set active via {@link #setNamingContext(boolean)}
     * and also has an id name set via {@link #setIdName(String)}.
     * 
    ...
    /**
     * A bean provider implementation which provides beans to the table repeater.
     * This provider takes the table's pagination state into account, so that only 
    ...
    private List<Integer> getRowIds(final int startIndex, final int endIndex)
    {
        ...
        {
            List<Integer> rowIds = new ArrayList<Integer>(endIndex - startIndex + 1);
    ...
    public static final class TableModel extends BeanAndProviderBoundComponentModel
    {
        ...
        /** 
         * This is used to map rendered table row indices to table model row indices, 
         * if the table model supports this mode of sorting. 
         */
         ...
         */
        private final Map<WComponent, List<ActionConstraint>> actionConstraints = new HashMap<WComponent, List<ActionConstraint>>(); 
        ...
        /**
         * The pagination row count takes into account different pagination types.
         * For example, hierarchical tables only paginate on top-level nodes (to avoid orphan nodes).
         * 
        ...
        public void addActionConstraint(final WButton button, final ActionConstraint constraint)
        {
            ...
            {
                constraintForComponent = new ArrayList<ActionConstraint>();
