...
/** 
 * <p>AbstractSearchReplaceWriter is a writer extension that allows programmatic replacement of
 * strings contained in written stream. For efficiency, it uses a buffer that is 
 * twice the size of the longest search string.</p>
 * 
 * <p>Data is only written to the underlying writer when the buffer is filled, or
 * when the writer is closed; calling {@link #flush()} only flushes the underlying writer. 
 * Calling {@link #close()} may not close the underlying writer, depending on the 
 * return value of {@link #closeBackingOnClose()}</p>
 * 
...
public abstract class AbstractSearchReplaceWriter extends Writer
{
    ...
    /**
     * The "lookahead" buffer. It buffers characters from the underlying stream to allow us 
     * to check for the "match" string before writing them to the underlying writer.
     */
    ...
    /**
     * Writes the current contents of the buffer, up to the given position.
     * More data may be written from the buffer when there is a search string
     * that crosses over endPos.
     * 
    ...
    /**
     * Searches for any search strings in the buffer that start
     ...
     * Subclasses can override this to keep the underlying writer open.
     * 
