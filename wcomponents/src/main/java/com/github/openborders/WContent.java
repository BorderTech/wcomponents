...
/**
 * <p>This wcomponent enables the display of arbitrary document content from a 
 * WComponent application. This component does not by default display anything 
 * until the {@link #display()} method is called. It then causes the 
 * browser to access the content through the WContentHelperServlet.</p> 
 * 
 * <p>By default, the browser will display the content in a new window, but the
 * {@link #setDisplayMode(DisplayMode)} method can be used to display the content 
 * inline or prompt the user to save the content to a file.</p>
 * 
...
public class WContent extends AbstractWComponent implements Targetable
{
    ...
    public static enum DisplayMode
    {
         ...
         * Note that for this to work, the ContenetAccess MUST have its description set to
         * the file name which the content should be saved as. 
         */
        PROMPT_TO_SAVE
    }
    ...
    /**
     * <p>Override handleRequest in order to perform processing specific to this component.</p>
     * 
     * <p>When the new browser window for the document content is opened, it
     * will make another request to fetch the content. It is that situation
     * we are trying to detect and handle here.</p>
     * 
