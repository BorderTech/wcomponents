...
import com.github.openborders.wcomponents.WDataTable.SelectMode;

/**
 * <p>This is component can be used to select all/none within a containing component, 
 * for example a {@link WPanel} or {@link WFieldSet} containing {@link WCheckBox}es, or 
 * a {@link WDataTable} with multiple row selection enabled.</p>
 * 
 * <p>Note that the target component must render out with an ID, so a 
 * plain {@link WComponent} or {@link WContainer} is not suitable.</p> 
 * 
...
public class WSelectToggle extends AbstractWComponent implements Disableable, AjaxTarget
{
    ...
    /**
     * Override handleRequest to handle selection toggling if
     * server-side processing is being used.
     * 
        ...
        else if (component instanceof WDataTable)
        {
            ...
            if (table.getSelectMode() == SelectMode.MULTIPLE)
            {
                if (selected)
                {
                    ...
                    List<Integer> indices = new ArrayList<Integer>(rowCount);
