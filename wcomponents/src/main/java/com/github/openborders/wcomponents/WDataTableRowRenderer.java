...
import com.github.openborders.wcomponents.WRepeater.SubUIContext;
import com.github.openborders.wcomponents.util.Util;

/**
 * WDataTableRowRenderer is used by the table's repeater to render row data.
...
public final class WDataTableRowRenderer extends WDataRenderer
{
    ...
    private final Map<Class<? extends WComponent>, WComponent> expandedRenderers = new HashMap<Class<? extends WComponent>, WComponent>(); 
    ...
    /**
     * <p>The preparePaintComponent method has been overridden to ensure that expanded
     * row renderers have been correctly initialised.</p>
     * 
     * <p>Expanded row renderers are lazily instantiated and added to the shared structure as  
     * needed. This means for the first use of a renderer, it will not have been part of the  
     * WComponent tree, and would not have had its preparePaintComponent called. We therefore 
     * add the renderer to the tree here, and manually call its preparePaint.</p>
     * 
    ...
    /** 
     * <p>This is called to lazily add expanded renderers as necessary.
     * To save memory, only one instance of a renderer class is ever added 
     * to the row renderer instance. The RendererWrapper ensures that data 
     * binding occurs at the right time.</p>
     * 
     ...
     * (ie. it is only involved in processing of certain rows), and ensuring that data is passed
     * to / from the renderer when required.
     */
    private static class RendererWrapper extends WBeanContainer implements BeanProvider
    {
         ...
         */
        public RendererWrapper(final WDataTableRowRenderer rowRenderer, final Class<? extends WComponent> rendererClass, final int columnIndex)
        {
         ...
         * @param columnIndex the index of the column renderer, or -1 if the renderer is for expanded content.
         */
        public RendererWrapper(final WDataTableRowRenderer rowRenderer, final WComponent renderer, final int columnIndex)
        {        
            this.rowRenderer = rowRenderer;
            this.columnIndex = columnIndex;
        ...
        public Object getBean(final BeanProviderBound beanProviderBound)
        {
            ...
            {
                row = rowRenderer.getRowIndex((SubUIContext) uic);
            }
            else
            {
        ...
        /**
         * Some renderers may not be bean provider bound, or not bean-aware.
         * We need to make sure that the data is set correctly for these columns.
         * 
         * @param request the request being responded to.
         */
        @Override
    ...
    /**
     * Adds a column to the renderer. This method is called by {@link WDataTable}
     * to keep the renderer's and table's columns in sync.
     * 
