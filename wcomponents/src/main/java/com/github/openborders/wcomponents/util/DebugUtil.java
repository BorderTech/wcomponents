...
/**
 * Utility class used by WComponents for accessing {@link Config configuration} parameters 
 * used in debug features.
 * 
...
final public class DebugUtil
{
    ...
    {
        debugFeaturesEnabled = Config.getInstance().getBoolean("wcomponent.debug.enabled", false);
        validateXMLEnabled = Config.getInstance().getBoolean("wcomponent.debug.validateXML.enabled", false);
        debugStructureEnabled = Config.getInstance().getBoolean("wcomponent.debug.clientSide.enabled", false);
    }
