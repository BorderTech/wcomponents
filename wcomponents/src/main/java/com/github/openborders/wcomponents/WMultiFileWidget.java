...
import com.github.openborders.wcomponents.WLink.ImagePosition;
import com.github.openborders.wcomponents.file.File;
import com.github.openborders.wcomponents.file.FileItemWrap;
import com.github.openborders.wcomponents.render.webxml.FileWidgetRendererUtil;
...
import com.github.openborders.wcomponents.util.thumbnail.ThumbnailUtil;
...
public class WMultiFileWidget extends AbstractInput implements Targetable, AjaxTrigger, AjaxTarget, SubordinateTarget
{
    ...
    public final static String FILE_UPLOAD_ID_KEY = "wc_fileid";

    /** File upload index thumb nail content request. */
    public final static String FILE_UPLOAD_THUMB_NAIL_KEY = "wc_filetn";
    ...
    public void addFile(final FileWidgetUpload file)
    {
        ...
        if (files == null)
        {
            files = new ArrayList<FileWidgetUpload>();
            setData(files);
        }
        files.add(file);
    }
     ...
     * For details on file types {@see #setFileTypes(Collection<String>)}
     *
     ...
     * Note that duplicates are not allowed and these are not case sensitive.
     * A file type may be one of the following:
     * <ul>
     ...
     * <li>A string whose first character is a "." (U+002E) character (Indicates that files with the specified file extension are accepted).</li>
     * </ul>
     * @param types The file types that will be accepted by the file input.
     * Note that this is not additive, it will overwrite any previously set fileTypes.
     * Pass null or and empty collection to clear all file types.
     */
    public void setFileTypes(final Collection<String> types)
    {
        ...
        if(types != null)
        {
            newFileTypes = new HashSet<String>();
     ...
     * For a description of what constitutes an allowable file type {@see #setFileTypes(Collection<String>)}}
     *
     * If no types have been added an empty list is returned. An empty list indicates that all file types are accepted.
     *
     * @return The file types accepted by this file input e.g. "image/*", ".vis", "text/plain", "text/html", "application/pdf".
     */
    public List<String> getFileTypes()
    {
        ...
        result = new ArrayList<String>(fileTypes);
        return result;
    }
    ...
    private List<FileWidgetUpload> getNewSelection(final Request request)
    {
        ...
        List<FileWidgetUpload> newSelection = new ArrayList<FileWidgetUpload>();
