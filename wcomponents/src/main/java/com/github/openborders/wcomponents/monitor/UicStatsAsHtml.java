package com.github.openborders.wcomponents.monitor; 
...
public final class UicStatsAsHtml
{
    ...
    private static void writeProfileForTree(final PrintWriter writer, final Map<WComponent, UicStats.Stat> treeStats)
    {
        ...
        List<UicStats.Stat> statList = new ArrayList<UicStats.Stat>(treeStats.values());
        ...
        Comparator<UicStats.Stat> comparator = new Comparator<UicStats.Stat>() 
        {
            ...
            {
                if (stat1.modelState > stat2.modelState)
                {
                    return -1;
                }
                else if (stat1.modelState < stat2.modelState)
                {
                    return 1;
                }
                else
                {
                    int diff = stat1.className.compareTo(stat2.className);
                    
                    if (diff == 0)
                    {
                        diff = stat1.name.compareTo(stat2.name);
                    }
    ...
    {
        writer.print("<tr>");
        writer.print("<td>" + stat.className + "</td>");
        writer.print("<td>" + stat.getModelStateAsString() + "</td>");
        
        if (stat.serializedSize > 0)
        {
            writer.print("<td>" + stat.serializedSize + "</td>");
        }
        else
        {
            writer.print("<td>&#160;</td>");
        ...
        writer.print("<td>" + stat.ref + "</td>");
        writer.print("<td>" + stat.name + "</td>");
        ...
        {
            writer.print("<td>" + stat.comment + "</td>");
        }
