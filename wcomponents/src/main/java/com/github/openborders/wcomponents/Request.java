 ...
 * <p>
 * The {@link com.github.openborders.wcomponents.servlet.WServlet} class is the generic servlet that sits between the servlet
 * container and the WComponent framework. The WServlet class is responsible for creating Request objects that represent
 * the incoming HTTP requests, and for dispatching them to the WComponent tree.
 *
 * @see com.github.openborders.wcomponents.servlet.WServlet
...
public interface Request
{
    ...
    /**
     * Returns the value of a request parameter as a <code>String</code>, or <code>null</code> if the parameter
     * does not exist. Request parameters are extra information sent with the request. For HTTP servlets, parameters are
    ...
    /**
     * <p>Returns the value of the named attribute as an <code>Object</code>, or <code>null</code> if no attribute of
     * the given name exists.</p>
     *
     * <p>Attributes can be set two ways. The container may set attributes to make available custom information about a
     * request. Attributes can also be set programatically using setAttribute.</p>
     *
     * @param key the request attribute key
     * @return an <code>Object</code> containing the value of the attribute, or <code>null</code> if the attribute
     *         does not exist
     */
     ...
     * If the object passed in is null, the effect is the same as removing the attribute.
     *
    ...
    /**
     * Returns the object bound with the specified name in the application session, or <code>null</code> if no object
    ...
    /**
     * Gets the parameter for the given key. If no such parameter is defined, returns null.
     * This method differs from Parameters.getInstance().get(key) in that the value returned
     * may be customised to suit the "app". In a portlet environment an "app" is represented
     * by a portlet.
     *
    ...
    /**
     * Signal to the servlet that we want to log out (terminate the session). The container
     * should react by invalidating the session and redirecting the client to some suitable page.
     *
     * @see com.github.openborders.wcomponents.container.AbstractContainerHelper#redirectForLogout()
     */
    void logout();
     ...
     * deployment descriptors. If the user has not been authenticated, the method returns false.
     *
