...
public class WAudio extends AbstractWComponent implements Targetable, AjaxTarget, Disableable
{
     ...
     * Audio clips must be added later by calling one of the setAudio(...) methods. 
     */
    public WAudio()
    {
    }
    ...
    /**
     * <p>Creates a WAudio with the given static content. This is provided as
     * a convenience method for when the audio file is included as static content
     * in the class path rather than in the web application's resources.</p>
     * 
     * <p>The mime type for the audio clip is looked up from the "mimeType.*" 
     * mapping configuration parameters using the resource's file extension.</p>
     *   
     ...
     * The client will try to load the first audio clip, and if it fails or 
     * isn't supported, it will move on to the next audio clip.
     * Only the first clip which can be played on the client will be used.
     * 
    ...
    /**
     * Creates dynamic URLs that the audio clips can be loaded from. In fact the URL points 
     * to the main application servlet, but includes a non-null for the parameter associated with 
     * this WComponent (ie, its label). The handleRequest method below detects this when the 
    ...
    /**
     * When an audio element is rendered to the client, the
     * browser will make a second request to get the audio content.
     * The handleRequest method has been overridden to detect whether the 
     * request is the "content fetch" request by looking for the parameter 
     * that we encode in the content url.
     * 
    ...
    /**
     * Sets which playback controls (e.g. stop/start/pause) to display on the audio component.
     * The values of {@link Controls#NONE} and {@link Controls#ALL} take precedence over all
     * other values. Passing a null or empty set of controls will cause the client's default
     * set of controls to be used.
     * 
