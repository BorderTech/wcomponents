 ...
 */
public class WPartialDateField extends AbstractInput implements AjaxTrigger, AjaxTarget, SubordinateTrigger,
    SubordinateTarget
{
    ...
    private final static char DEFAULT_PADDING_CHAR = ' ';

    /** Character used by the theme to pad the partial date value. */
    private final static char THEME_PADDING_CHAR = '?';

    /** Year start position. */
    private final static int YEAR_START = 0;
    /** Year end position. */
    private final static int YEAR_END = 4;
    /** Month start position. */
    private final static int MONTH_START = 4;
    /** Month end position. */
    private final static int MONTH_END = 6;
    /** Day start position. */
    private final static int DAY_START = 6;
    /** Day end position. */
    private final static int DAY_END = 8;

    /** Internal Format - Year start position. */
    private final static int INTERNAL_YEAR_START = 0;
    /** Internal Format - Year end position. */
    private final static int INTERNAL_YEAR_END = 4;
    /** Internal Format - Dash 1. */
    private final static int INTERNAL_DASH1_POS = 4;
    /** Internal Format - Month start position. */
    private final static int INTERNAL_MONTH_START = 5;
    /** Internal Format - Month end position. */
    private final static int INTERNAL_MONTH_END = 7;
    /** Internal Format - Dash 2. */
    private final static int INTERNAL_DASH2_POS = 7;
    /** Internal Format - Day start position. */
    private final static int INTERNAL_DAY_START = 8;
    /** Internal Format - Day end position. */
    private final static int INTERNAL_DAY_END = 10;
    /** Internal Format - Number of digits in the date. */
    private final static int INTERNAL_DATE_TOTAL_CHARS = 10;

    /** Number of digits in the date. */
    private final static int DATE_TOTAL_CHARS = 8;
    /** Number of digits in the year. */
    private final static int YEAR_DIGITS = 4;
    /** Number of digits in the month. */
    private final static int MONTH_DIGITS = 2;
    /** Number of digits in the year. */
    private final static int DAY_DIGITS = 2;
    ...
    private final static int YEAR_MIN = 0;
    ...
    public WPartialDateField(final Integer day, final Integer month, final Integer year)
    {
        ...
        if (!isValidYear(year))
        {
            throw new IllegalArgumentException("Invalid partial year value (" + year + "). Year should be between "
        ...
        {
            throw new IllegalArgumentException("Invalid partial month value (" + month + "). Month should be between "
        ...
        if (!isValidDay(day))
        {
            throw new IllegalArgumentException("Invalid partial day value (" + day + "). Day should be between "
                                               + DAY_MIN + " to " + DAY_MAX + ".");
        }
    ...
    public String getRequestValue(final Request request)
    {
        if (isPresent(request))
        {
            ...
            if (dateParam.length() != INTERNAL_DATE_TOTAL_CHARS || dateParam.charAt(INTERNAL_DASH1_POS) != '-'
    ...
    public String getValue()
    {
        ...
        {
            throw new SystemException("PartialDate data value (" + value + ") is not in the format yyyyMMdd");
        }
     ...
     */
    private String formatPartialDateToString(final Integer day, final Integer month, final Integer year,
                                             final char padding)
    {
    ...
    private boolean isValidPartialDateStringFormat(final String component, final char padding)
    {
        ...
        {
            month = component.substring(MONTH_START, MONTH_END);
            if (!isValidCharacters(month, padding) || !isValidMonth(parseDateComponent(month, padding)))
            {
                return false;
            }
        }
        ...
        {
            day = component.substring(DAY_START, DAY_END);
     ...
     */
    private void append(final StringBuffer buf, final Integer num, final int digits, final char padding)
    {
        if (num == null)
        {
