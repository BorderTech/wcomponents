...
/**
 * <p>Interceptor components are used to plug in features to a container that
 * is serving WComponents. Interceptor components can be chained together.
 * The last component in the chain is normally the WComponent (UI) that is
 * being served up.</p>
 * 
 * <p>A new interceptor chain is created for each request that is served, so
 * interceptors do not need to be thread safe. Note that in a Portal environment,
 * the action and render phases will have separate interceptor chain instances.</p>
 * 
...
public class InterceptorComponent implements WebComponent
{
     ...
     */
    public void attachResponse(final Response response)
    {
        this.response = response;
        if (backing instanceof InterceptorComponent)
        {
            ((InterceptorComponent) backing).attachResponse(response);
        }
    }
     ...
     * @return the modified interceptor chain. If no match was found, the existing interceptor chain is returned unchanged. 
     */
