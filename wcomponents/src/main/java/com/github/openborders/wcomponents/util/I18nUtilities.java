...
public final class I18nUtilities
{
    ...
    public static final String RESOURCE_BUNDLE_BASE_NAME_CONFIG_KEY = "wcomponent.i18n.baseName";
    
    /** A store of bad bundles, to avoid repeated logging of errors when the bundle fails to load. */
    private static final Set<Locale> MISSING_RESOURCES = new HashSet<Locale>(); 
    ...
    /**
     * Retrieves the resource bundle base name. The base name is looked up
     * using the parameter {@link #RESOURCE_BUNDLE_BASE_NAME_CONFIG_KEY}.
     * 
    ...
    /**
     * <p>Attempts to format the given message for the given locale.</p>
     * 
     * <p>If a locale is provided, this method will attempt to load the text
     * from a resource bundle. If <code>message</code> is a {@link Message} 
     * and has format arguments, then {@link MessageFormat} will be used to 
    ...
    public static String getLocalisedText(final Locale locale, final String text)
    {
        ...
        if (!Util.empty(resourceBundleBaseName))
        {
            ...
            catch (MissingResourceException e)
            {
                ...
                if (text != null && text.startsWith("wcomponent.message."))
                {
        ...
        else if (text != null && text.startsWith("wcomponent.message."))
        {
            // Fall back to the Configuration mechanism for the default internal messages
            message = Config.getInstance().getString(text, null);
        }
