...
/**
 * WhiteSpaceFilterPrintWriter is a writer that filters 
 * out extraneous whitespace from HTML content written to it.
 * 
...
{    
    ...
    public void write(final int c)
    {
        ...
        if (change.outputBytes != null)
        {
            for (int i = 0; i < change.outputBytes.length; i++)
            {
                super.write(change.outputBytes[i]);
            }
        }
        
        if (!change.suppressCurrentChar)
        {
            super.write(c);
        }
    }
