...
public final class VelocityEngineFactory
{
    ...
    /**
     * If this setting is non-null, velocity templates will be loaded from
     * the given directory. This is good for developers, who can point into
     * their source tree directly. Templates will not be cached in this case.
     */
    private static final String FILE_TEMPLATES_KEY = "wcomponent.velocity.fileTemplatesDir";

    /**
     * If we are not using fileTemplates, the templates are read from the
     * classpath. This setting governs whether we cache these templates or
     * not. For production, we should cache.
     */
    private static final String CACHE_TEMPLATES_KEY = "wcomponent.velocity.cacheTemplates.enabled";
    ...
    /**
     * <p>Returns the VelocityEngine associated with this factory.
     * If this is the first time we are using the engine, create it
     * and initialise it.</p>
     * 
     * <p>Note that velocity engines are hugely resource intensive, so we don't
     * want too many of them. For the time being we have a single instance
     * stored as a static variable. This would only be a problem if the
     * VelocityLayout class ever wanted to use different engine configurations
     * (unlikely).</p>
     * 
    ...
    public static synchronized VelocityEngine getVelocityEngine()
    {
        if (engine == null)
        {
            ...
            props.setProperty(RuntimeConstants.RUNTIME_LOG_LOGSYSTEM_CLASS,
                "com.github.openborders.wcomponents.velocity.VelocityLogger");
            
            // Set up access to the common velocity macros.
            props.setProperty(RuntimeConstants.VM_LIBRARY, config.getString("wcomponent.velocity.macroLibrary"));
