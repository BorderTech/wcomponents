...
import com.github.openborders.wcomponents.NamingContextable;
...
import com.github.openborders.wcomponents.WInvisibleContainer;
import com.github.openborders.wcomponents.WRepeater;
import com.github.openborders.wcomponents.WWindow;
import com.github.openborders.wcomponents.WebUtilities;
import com.github.openborders.wcomponents.WRepeater.WRepeatRoot;
import com.github.openborders.wcomponents.util.WComponentTreeVisitor.VisitorResult;
...
public final class TreeUtil
{
    ...
    {
        final List<ComponentWithContext> list = new ArrayList<ComponentWithContext>();
     ...
     * @return the component with the given id, or null if not found.
     */
    public static WComponent getComponentWithId(final WComponent root, final String id, final boolean visibleOnly)
    {
    ...
    public static UIContext getContextForId(final WComponent root, final String id, final boolean visibleOnly)
    {
        AbstractTreeVisitorWithResult<UIContext> visitor = new AbstractTreeVisitorWithResult<UIContext>()
        {
            ...
            public VisitorResult visit(final WComponent comp)
            {
                ...
                if (id.equals(comp.getId()))
                {
                    result = UIContextHolder.getCurrent();
                    return VisitorResult.ABORT;
                }
                                                                    ...
                                                                    final boolean visibleOnly)
    {
        AbstractTreeVisitorWithResult<ComponentWithContext> visitor = new AbstractTreeVisitorWithResult<ComponentWithContext>()
        {
            ...
            public VisitorResult visit(final WComponent comp)
            {
                ...
                if (id.equals(comp.getId()))
                {
                    result = new ComponentWithContext(comp, UIContextHolder.getCurrent());
                    return VisitorResult.ABORT;
                }
    ...
    {
        List<ComponentWithContext> matchAtLevel = new ArrayList<ComponentWithContext>();
        ...
        for (int i = 0; i < path.length; i++)
        {
            ...
            matchAtLevel = new ArrayList<ComponentWithContext>();
            ...
            for (ComponentWithContext comp : matchAtLastLevel)
            {
                ...
                FindComponentByClassVisitor visitor = new FindComponentByClassVisitor(comp, className, i == 0);
                ...
                if (index >= 0)
                {
                    if (index < visitor.result.size())
                    {
                        matchAtLevel.add(visitor.result.get(index));
                    }
                }
                else
                {
                    matchAtLevel.addAll(visitor.result);
                }
            }
        }
        ...
        AbstractTreeVisitorWithResult<List<ComponentWithContext>>
    {
        ...
        private final List<ComponentWithContext> result = new ArrayList<ComponentWithContext>();
         ...
         */
        public FindComponentByClassVisitor(final ComponentWithContext root, final String className,
        ...
        public VisitorResult visit(final WComponent comp)
        {
            ...
            {
                if (classNamesMatch(clazz.getName(), className) && (includeRoot || comp != root.getComponent()))
                {
                    result.add(new ComponentWithContext(comp, UIContextHolder.getCurrent()));
                }
            }
