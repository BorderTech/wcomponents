 ...
 * This class is abstract, as it doesn't make sense to instantiate a
...
public abstract class AbstractTreeNode implements TreeNode, Serializable
{
    ...
    private static final Iterator<TreeNode> EMPTY_ITERATOR = new EmptyIterator<TreeNode>();
     ...
     * 
     * @param aNode
     *            the DefaultTreeNode to get the path for
     * @param depth
    ...
    private static final class BreadthFirstIterator implements Iterator<TreeNode>
    {
         ...
         * 
         * @param rootNode
         *            the root node for the iterator.
         */
        public BreadthFirstIterator(final TreeNode rootNode)
        {
            super();
            List<TreeNode> list = new ArrayList<TreeNode>(1);
            list.add(rootNode);
            queue = new Queue<Iterator<TreeNode>>();
        ...
        private static final class Queue <T>
        {
            ...
            private static final class QNode <T>
            {
                 ...
                 */
                public QNode(final T object, final QNode<T> next)
                {
                    this.object = object;
                    this.next = next;
                }
            ...
            public void enqueue(final T anObject)
            {
                if (head == null)
                {
                    head = new QNode<T>(anObject, null);
                    tail = head;
                }
                else
                {
                    tail.next = new QNode<T>(anObject, null);
                    tail = tail.next;
                }
            }
    ...
    private static final class PostorderIterator implements Iterator<TreeNode>
    {
         ...
         * 
         * @param rootNode
         *            the root node for this iteration.
         */
        public PostorderIterator(final TreeNode rootNode)
        {
            root = rootNode;
