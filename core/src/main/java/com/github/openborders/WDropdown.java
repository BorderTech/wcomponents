...
/**
 * <p>The WDropdown component is used to let the user select a single option from a drop-down list. 
 * The list of options that can be selected are supplied at construction time as a parameter
 * in the constructor or via the {@link #setOptions(List)} method. The list of options are java objects that are 
 * rendered using their toString() by default.</p>
 * 
 * <p>Use the {@link #getSelected() getSelected} method to determine which of the list of options 
 * was chosen by the user. Note that getSelected returns one of the object instances supplied 
 * in the original list of options.</p>
 * 
 * <p>The WDropdown component supports extensions using the {@link #setType(DropdownType)} method. 
 * The {@link DropdownType#COMBO "combo"} drop-down extension allows the user to enter in a value
 * which was not present in the original list.</p>   
 *
...
public class WDropdown extends AbstractWSingleSelectList implements AjaxTrigger, AjaxTarget, SubordinateTrigger, SubordinateTarget
{
    ...
    /**
     * Indicates whether users are able to enter in an arbitrary value,
     * rather than having to pick one from the drop-down list.
     * 
    ...
    /**
     * Sets the type of this drop down.  
     * If un-set, reverts to the un-editable/native type. 
     * 
    ...
    /**
     * Set the width of the selectable options when rendered to screen.
     * Has no effect with a native drop down.
     * 
